...
uint32_t MCI_Init(uint8_t powerActiveLevel )
{
	...
	retval = MCI_CardInit();
    (void)retval;   // remove the fucking warning!!
...
uint32_t MCI_Cmd_SendOpCond( void )
{
	...
	while ( retryCount > 0 )
	{
		...
		{
			retval = (uint32_t)MCI_FUNC_TIMEOUT;
		}
		else if ((respValue[0] & 0x80000000) == 0)
		{
			//The card has not finished the power up routine
			retval = (uint32_t)MCI_FUNC_BUS_NOT_IDLE;
		}
		else
		{
			retval = (uint32_t)MCI_FUNC_OK;
			break;
		}
	...
	while ( retryCount > 0 )
	{
		...
		else if ((respValue[0] & MCI_CMD8_CHECKPATTERN_BMASK) != checkPattern)
		{
			retval = MCI_FUNC_FAILED;
						...
						!= voltageSupplied)
		{
			retval = (uint32_t)MCI_FUNC_BAD_PARAMETERS;
		}
		else
		{
			retval = MCI_FUNC_OK;
...
{
	uint32_t i, retryCount;
	uint32_t respStatus, argument = 0;
	uint32_t respValue[4];
...
uint32_t MCI_CardInit( void )
{
	...
	if(retval == (uint32_t)MCI_FUNC_BAD_PARAMETERS)
	{
...
uint32_t MCI_GetCID(st_Mci_CardId* cidValue)
{
	...
	return (uint32_t)MCI_FUNC_TIMEOUT;
}
